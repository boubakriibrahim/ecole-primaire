            ...
            <div class="p-3">
              <table id="example1" class="table table-bordered table-striped text-right p-3">
                ...
                <tr>
                  <td>
                      <div class="row">
                        <a href="{{ route("salle.delete", $salle->id) }}" class="btn btn-danger offset-sm-3 mr-sm-1 col-sm-4 order-3 order-sm-1" id="delete">حذف</a>

                        <button type="button" class="btn btn-info  col-sm-4 order-2" data-toggle="modal" data-target="#model{{$salle->id}}">
                            تحديث
                          </button>
                        </div>
                  </td>
              ...
              <!-- Modal -->
              <div class="modal fade" id="model{{$salle->id}}" tabindex="1" aria-labelledby="exampleModal{{$salle->id}}" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header text-center">
                    ...
                    <div class="modal-body">
                        <form method="post" action='{{ route('salle.miseajour',$salle->id) }}' width=60%>
                            @csrf


                                  <div class="form-group text-right">
                                    <label for="libelle text-right"><span class="text-danger">*</span> الإسم</label>
                                    <input type="text" class="form-control text-right" id="libelle" name="libelle" placeholder="أدخل الإسم" value="{{ $salle->libelle }}" required>
                                  </div>
